...
<div class="mt-4 flex flex-col items-center justify-center">
    ...
    <div class="w-1/2 items-center justify-center space-y-4">
        <form class="space-y-3 rounded-md border-2 border-secondary-500 p-4">
            ...
            <!-- ToDo format Date -->
            <button class="btn variant-filled-primary w-full" on:click={() => {formPage = 1; saveSelected(beginReference.value,endReference.value,stations[selectedStationIndex]); fetchAvailableAllCars(stations);}}>Weiter</button>
            {:else if formPage == 1}
            {#await cars then loadedCars}
            <label class="label" for="car_table">Verfügbares Auto auswählen</label>
            <div class="rounded-lg flex flex-col p-2 space-y-4">
                {#if loadedCars[selectedCarIndex]}
                    {#each loadedCars as car, index}
                    <a href="/new-reservation"
                    on:click|preventDefault={() => {document.getElementById(`radio_${index}`).checked = true;}}>
                    <div class="grid grid-cols-6 bg-surface-600 p-2 rounded-lg hover:bg-surface-500">
                        <div class="col-span-2">
                            <img src="carImages/{car.model}.webp" alt="car pic" class="rounded-lg">
                        </div>
                        <div class="col-span-3 p-2 flex flex-col items-center justify-center h-full">
                        <table id="car_table" class="text-left border-separate border-spacing-x-2 w-full">
                            <tr>
                                <th>
                                    Marke:
                                </th>
            ...
            <div class="grid grid-cols-2 space-x-2">
                <button class="btn variant-filled-primary" on:click={() => {formPage = 0;}}>Zurück</button>
                <button class="btn variant-filled-primary" on:click={() => {if(checkAndSaveCarSelected(loadedCars)){formPage = 2;}}}>Weiter</button>
            </div>
            {/await}
            {:else if formPage == 2}
            <!-- Reservation summary -->
             ...
             <div class="grid grid-cols-2">
                <img src="carImages/{selectedCar.model}.webp" alt="car pic" class="rounded-lg">
            <div class="p-2 flex flex-col items-center justify-center h-full">
             <table class="border-separate border-spacing-x-2 text-left">
                <tr>
                    <th>
                        Beginn:
                    </th>
             ...
             <div class="grid grid-cols-2 space-x-2">
                <button class="btn variant-filled-primary" on:click={() => {formPage = 1;}}>Zurück</button>
                <button class="btn variant-filled-primary" on:click={() => {createReservation()}}>Reservierung bestätigen</button>
            </div>
            {/if}
        </form>
    </div>
